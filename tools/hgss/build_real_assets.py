...
APICULA_VERSION='apicula-2026-08-26-2'
...
def convert_map_models():
    ...
    for map_id in [x.strip() for x in MAPS.split(',') if x.strip()]:
        ...
        # Apicula refuses an existing output directory. Remove any stale output
        # from extraction or a previous conversion, then let Apicula create it.
        if outdir.exists():
