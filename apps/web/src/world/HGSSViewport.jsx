...
export default function HGSSViewport({mapId=30,name='Route 29',connections=[],onChange}){
 const ref=useRef(null),[status,setStatus]=useState('Chargement de la carte…'),[zoom,setZoom]=useState(1),[offset,setOffset]=useState({x:0,y:0}),[hasReal,setHasReal]=useState(false);
 const seed=Number(mapId)||30;
 const tiles=useMemo(()=>Array.from({length:COLS*ROWS},(_,i)=>{const x=((seed*17+i*7)%8)*32;const y=((seed*31+i*13)%65)*32;return{x,y}}),[seed]);
 useEffect(()=>{
  ...
  const renderer=new THREE.WebGLRenderer({antialias:false});renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,2));host.appendChild(renderer.domElement);
  const loader=new GLTFLoader();let model=null;
  const base=import.meta.env.BASE_URL||'/';
  loader.load(`${base}assets/hgss/generated/maps/${seed}/rendered/nsbmd.glb`,g=>{model=g.scene;scene.add(model);setHasReal(true);setStatus(`HGSS réel · ${name} · map ${seed}`)},undefined,()=>setStatus(`Aperçu carte actif · ${name} · map ${seed}`));
  scene.add(new THREE.HemisphereLight(0xffffff,0x44505c,2));
  ...
  resize();window.addEventListener('resize',resize);let raf=requestAnimationFrame(function tick(){renderer.render(scene,camera);raf=requestAnimationFrame(tick)});
 ...
 return <section className="hgss-play">
  ...
  <div className="hgss-canvas" ref={ref} style={hasReal?transform:undefined}/>
  {!hasReal&&<div className="hgss-fallback" style={transform} aria-label="Aperçu de la carte HGSS">{tiles.map((t,i)=><i key={i} style={{backgroundImage:`url(${(import.meta.env.BASE_URL||'/')}assets/titleset1.png)`,backgroundPosition:`-${t.x}px -${t.y}px`}}/>)}<strong>{name}</strong></div>}
