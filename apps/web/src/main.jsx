...
function CharacterSprite({src,palette="npc-blue",direction="down"}){const row={down:0,left:1,right:2,up:3}[direction]??0;return <span className={`character-sprite ${palette}`}style={{backgroundImage:`url(${src})`,backgroundSize:"96px 128px",backgroundPosition:`-32px -${row*32}px`}}/>}
function Tile({type,x,y,onPaint}){const p=TILE_POS[type]||TILE_POS.grass;return <div className={`tile tile-${type}`}style={{left:x*TILE,top:y*TILE,backgroundColor:FALLBACK[type],backgroundImage:`url(${TILESET})`,backgroundSize:"512px 4160px",backgroundPosition:`-${p[0]*TILE}px -${p[1]*TILE}px`}}onPointerDown={onPaint?e=>{e.stopPropagation();onPaint(x,y)}:undefined}/>}
