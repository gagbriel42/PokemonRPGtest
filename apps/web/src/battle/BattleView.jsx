...
function Sprite({pokemon,side}){const[attempt,setAttempt]=useState(0);const candidates=getHgssSpriteCandidates(pokemon.name,side);const src=candidates[attempt];return <div className={`hgss-sprite ${side}`}>
 ...
 <small>{candidates.length?'HGSS':'sprite local à importer'}</small>
 </div>}
...
export default function BattleView({onExit}){
 ...
 return <section className="battle-screen hgss-battle"><header className="battle-topbar hgss-battle-top"><div><small>COMBAT · INTERFACE JOUEUR</small><h2>Combat Pokémon</h2></div><div className="battle-mode"><b>JOUEUR</b><span>interface inspirée HGSS</span></div><button onClick={onExit}>QUITTER</button></header>
 <div className="battle-field hgss-field"><div className="hgss-sky"/><div className="hgss-ground enemy-ground"/><div className="hgss-ground player-ground"/><Fighter pokemon={activeEnemy} enemy/><Fighter pokemon={activePlayer}/><div className="battle-name enemy-name">PNJ · {activeEnemy.name}</div><div className="battle-name player-name">VOUS · {activePlayer.name}</div></div>
 ...
 {panel==='bag'&&<div className="battle-subpanel hgss-moves"><b>SAC</b><button onClick={()=>{setPlayer(t=>t.map((p,i)=>i===selected?{...p,hp:Math.min(p.maxHp,p.hp+10)}:p));setMessage('Potion utilisée : +10 PV');setPanel(null)}}><strong>Potion</strong><small>+10 HP</small></button></div>}
